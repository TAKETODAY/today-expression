...
public final class ExpressionBuilder implements NodeVisitor {
	...
	private final static Node createNodeInternal(String expr) throws ELException {
		...
		if (n == null) {
			try {
				n = (new ELParser(new ELParserTokenManager(new SimpleCharStream(new StringReader(expr), 1, 1, expr.length() + 1))))//
						.CompositeExpression();
	...
	public void visit(Node node) throws ELException {
		if (node instanceof AstFunction) {
			...
			if ((funcNode.getPrefix().length() == 0) && (this.fnMapper == null || fnMapper.resolveFunction(
					funcNode.getPrefix(),
